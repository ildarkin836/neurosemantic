...
class Detector(Wrapper):
    ...
    def preprocess(self, image: np.ndarray, keep_ratio: bool = True) -> List[np.ndarray, int, int, int, int]:
        """
        ...
        if keep_ratio and image.shape[0] != image.shape[1]:
            ...
            if hw_scale > 1:
                ...
                img = cv2.copyMakeBorder(img, 0, 0, left, self.input_width - new_w - left, cv2.BORDER_CONSTANT,
                                         value=(0, 0, 0))  # add border
            else:
                new_h, new_w = int(self.input_height * hw_scale), self.input_width
                img = cv2.resize(image, (new_w, new_h), interpolation=cv2.INTER_AREA)
                top = int((self.input_height - new_h) * 0.5)
                img = cv2.copyMakeBorder(img, top, self.input_height - new_h - top, 0, 0, cv2.BORDER_CONSTANT,
                                         value=(0, 0, 0))
        else:
            img = cv2.resize(image, (self.input_width, self.input_height), interpolation=cv2.INTER_AREA)
    ...
    def postprocess(self, 
                   tensor: np.ndarray, scale_h: float, scale_w: float, padh: int, padw: int) -> np.ndarray:
        """
    ...
    def __call__(self, tensor: np.ndarray):
        ...
        det_bboxes, det_conf, det_classid, landmarks = self.postprocess(tensor, scale_h, scale_w, top, left)
...
if __name__ == '__main__':
    ...
    cv2.imwrite('res.jpg', detector.draw_detections(img, det_bboxes, det_conf, landmarks))
